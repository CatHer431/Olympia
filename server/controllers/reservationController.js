...
const getUserReservation = async (req, res) => {
    try {
        const email = req.query.email;
        console.log("Ready: " + email);
        const payments = await Payment.find({ email: email, status: 1 }).exec();
        const result = await Promise.all(payments.map(async (payment) => {
            const reservation = await Reservation.getById(payment.reservation_id);
            if (reservation) {
                return reservation;
            }
        })).then(reservations => {
            // Filter out null reservations
            return reservations.filter(reservation => reservation !== undefined && reservation !== null);
        }).catch(err => {
            console.error(err);
            return [];
        });
...
module.exports = { newReservation, getReservation, cancelReservation, changeReservation, getReservationByEmail, getUserReservation };
